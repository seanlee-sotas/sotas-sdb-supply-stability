...
SUMITOMO_OVERRIDES_P = Path(__file__).resolve().parent / "sumitomo_overrides.yml"


@lru_cache(maxsize=1)
def _sumitomo_overrides_by_cas() -> dict[str, dict]:
    """CAS-level overrides from app/sumitomo_overrides.yml (auto-generated by sumitomo_data_expand).

    Allows the sumitomo mock to inject wb_commodity / sec_tickers / hs6 into
    scoring's chem dict without modifying legacy materials.yml.
    """
    if not SUMITOMO_OVERRIDES_P.exists():
        return {}
    try:
        doc = yaml.safe_load(SUMITOMO_OVERRIDES_P.read_text(encoding="utf-8")) or {}
...
def get_chemical(cas: str) -> dict | None:
    ...
    sumi = _sumitomo_overrides_by_cas().get(cas, {})
    # sumitomo overrides take precedence when legacy is empty
    row["sec_tickers"] = legacy.get("sec_tickers") or sumi.get("sec_tickers") or []
    row["wb_commodity"] = legacy.get("wb_commodity") or sumi.get("wb_commodity")
    row["capacity_keywords"] = legacy.get("capacity_keywords") or []
    row["name_ja_legacy"] = legacy.get("name_ja") or sumi.get("name_ja")
    # Merge sumitomo hs6 into hs6_exact (if not already present from hs_map.parquet)
    sumi_hs6 = sumi.get("hs6")
    if sumi_hs6 and sumi_hs6 not in row["hs6_exact"]:
        row["hs6_exact"].append(str(sumi_hs6))
