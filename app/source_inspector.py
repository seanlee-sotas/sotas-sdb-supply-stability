...
DATASETS: dict[str, dict] = {
    ...
    "jpca_utilization": {
        "title": "JPCA 月次 エチレンクラッカー実質稼働率",
        "source_url": "https://www.jpca.or.jp/statistics/monthly/memo.html",
        "license": "石油化学工業協会 (公的データ)",
        "cadence": "月次 (memo PDF 取得 + regex 抽出)",
        "ingest_script": "ingest/jpca_utilization.py",
        ...
        "columns": {
            ...
            "period": "YYYY-MM",
            "util_current": "当月 稼働率 (%)",
            "util_prev_month": "前月 稼働率 (%)",
            "util_prev_year_same_month": "前年同月 稼働率 (%)",
            "teishu_current": "当月 定修プラント (社/プラント)",
            "teishu_prev_month": "前月 定修プラント",
            "teishu_prev_year_same_month": "前年同月 定修プラント",
            "ethylene_kton": "当月エチレン生産量 (千トン)",
            "ethylene_mom_pct": "前月比 %",
            "ethylene_yoy_pct": "前年同月比 %",
            "memo_url": "出典 PDF URL",
            "_fetched_at": "取得日時 (UTC)",
        },
    },
    "chem_daily": {
        "title": "化学工業日報 (chemicaldaily.com) ヘッドライン",
        "source_url": "https://chemicaldaily.com/sitemap_index.xml",
        "license": "化学工業日報社 (公開メタデータ、本文は購読制)",
        "cadence": "実行ごと (URL差分追加、直近90日窓)",
        "ingest_script": "ingest/chem_daily_sitemap.py",
        "axes": ["軸1 短期要因 (直近90日)"],
        "columns": {
            "url": "記事URL (/archives/<id>)",
            "article_id": "WordPress post ID",
            "title": "見出し (og:title)",
            "summary": "冒頭抜粋 (og:description)",
            "pub_date_iso": "記事の公開日時 (ISO 8601, article:published_time)",
            "sitemap_lastmod": "sitemap 上の最終更新時刻",
            "_fetched_at": "取得日時 (UTC)",
        },
    },
    "chem_news": {
        "title": "化学業界 disruption ニュース (Google News RSS)",
        "source_url": "https://news.google.com/rss/search",
        "license": "Google News (公開フィード, 元記事の著作権は各社)",
        "cadence": "実行ごと (重複URLは除外)",
        "ingest_script": "ingest/chem_news_rss.py",
        "axes": ["軸1 短期要因 (直近90日) / 軸6 過去の供給途絶 (補完)"],
        "columns": {
            "title": "記事タイトル",
            "link": "Google News redirect URL",
            "pub_date": "公開日 (RFC2822)",
            "pub_date_iso": "公開日 (ISO 8601 UTC)",
            "source_name": "発信メディア (Yahoo!ニュース, 日経等)",
            "source_url": "発信メディアの基底URL",
            "description_html": "RSS description (HTML エスケープ含)",
            "query_name": "ヒットしたクエリ名 (plant_accident, supply_halt, recall, naphtha, ethylene_cracker, global_chem_disruption)",
            "query_q": "クエリ実体",
            "query_lang": "ja / en",
            "_fetched_at": "取得日時 (UTC)",
        },
